...
| 49 | Xie et al., *Adan* (arXiv:2208.06677) | `NdAdan` : momentum de **Nesterov adaptatif** (différences de gradients) ; CLI `lm --opt adan` | `nn::nd_optim` | 📋 | M |
...
| 54 | Sun et al., *RetNet : Retentive Network* (arXiv:2307.08621) | `NdRetention` : rétention (formes parallèle/récurrente équivalentes — oracle d'égalité) + décroissance | `nn::nd_layers` | 📋 | L |
| 55 | Yang et al., *Gated Linear Attention (GLA)* (ICML 2024, arXiv:2312.06635) | `gated_linear_attention` : attention linéaire **gatée** (porte dépendante de l'entrée), déroulée sur la tape ; gradient check | `nn::nd_layers` | 📋 | L |
...
| 58 | Qin et al., *HGRN : Hierarchically Gated RNN* (NeurIPS 2023, arXiv:2311.04823) | `NdHgrn` : RNN linéaire à **portes hiérarchiques** (borne inférieure croissante par couche) | `nn::nd_layers` | 📋 | M |
