...
- `scirust kvcache [--seed N] [--budget B]` —— 弾力的な圧縮 KV キャッシュ（INT4 のベース＋残差タイル、グループごとのスケール）；圧縮率と、全精度に対するアテンションの余弦忠実度を報告し、`--budget` で有界なソフトページング（SLHAv2/CCOS の相乗効果）を表示。
- `scirust guard [--seed N] [--alpha A]` —— 統計的に保証されたガード；分布非依存の共形被覆率（≥ 1−α）に加え、Accept/Abstain/Reject の判定（CCOS の応答ガード向け）。
- `scirust attest [--seed N]` —— 検証可能な推論のハッシュ連鎖アテステーションログ（Freivalds、#80）；連鎖を検証し、偽造された推論を棄却し、改ざん耐性を示す（CCOS のイベントログ連携）。
- `scirust quantum [--seed N] [--qubits Q] [--chi C]` —— MPS / Tensor-Train 量子回路シミュレータ（自製の打ち切り SVD、FFI なし）；厳密な GHZ 状態を表示し、密な 2ⁿ 状態ベクトルと比較してボンド次元と MPS のメモリ使用量を報告。
