...
fn rosenbrock(x: &[f64]) -> f64 {
    (0..x.len().saturating_sub(1)).map(|i| 100.0 * (x[i + 1] - x[i] * x[i]).powi(2) + (1.0 - x[i]).powi(2)).sum()
}
fn ackley(x: &[f64]) -> f64 {
    let n = x.len() as f64;
...
fn ga_min(f: fn(&[f64]) -> f64, dims: usize, gens: usize) -> f64 {
    ...
    for _ in 0..gens {
        ga.evolve(&mut pop, |inds| inds.iter().map(|ind| -f(&ind.genome)).collect());
    }
    pop.iter().map(|ind| f(&ind.genome)).fold(f64::INFINITY, f64::min)
}
fn cmaes_min(f: fn(&[f64]) -> f64, dims: usize, steps: usize) -> f64 {
    ...
    for _ in 0..steps {
        ...
        for ind in &off { let v = f(&ind.genome); if v < best { best = v; } }
        let v = f(&theta); if v < best { best = v; }
    }
    best
}
...
fn main() {
    ...
    println!();
    println!("=== AUDIT evo : oracle = optimum connu (min=0), dim={}, {} runs ===", dims, runs);
    println!("{:<11} {:>7}  {:>21}  {:>21}  {:>21}", "fonction", "optimum", "GA best/mean", "CMA-ES best/mean", "OpenES best/mean");
    for &(name, f) in &funcs {
        ...
        println!("{:<11} {:>7.1}  {:>9.2e}/{:>9.2e}  {:>9.2e}/{:>9.2e}  {:>9.2e}/{:>9.2e}",
