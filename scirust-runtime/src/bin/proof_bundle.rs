...
fn show(title: &str, checks: &[(String, bool)]) -> bool {
    ...
    println!("  => {}", if all { "BUNDLE VALIDE" } else { "BUNDLE REJETE" });
    all
}
fn main() {
    let mut r = R(12345);
    let model = QModel { layers: vec![lin(&mut r, 784, 256, 0.02, true), lin(&mut r, 256, 10, 0.03, false)] };
    ...
    println!("=== BUNDLE DE PREUVES ({} octets, ecrit dans {}) ===", text.len(), path);
    print!("{text}");
    println!();

    let ok_clean = show("=== VERIFICATION (artefact authentique) ===", &proof::verify_file(&text, &artifact));
    println!();

    let mut bad = artifact.clone();
    let idx = 200usize;
    bad[idx] ^= 0xFF;
    let ok_bad = show(&format!("=== VERIFICATION (artefact altere: octet {} retourne) ===", idx), &proof::verify_file(&text, &bad));
    println!();
    println!("RESUME: authentique={}  altere_rejete={}", ok_clean, !ok_bad);
}
