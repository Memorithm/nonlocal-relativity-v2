...
- `scirust kvcache [--seed N] [--budget B]` —— 弹性压缩 KV 缓存（INT4 基础块 + 残差块，按组缩放）；报告压缩比以及注意力相对全精度的余弦保真度，`--budget` 展示有界软分页（SLHAv2/CCOS 协同）。
- `scirust guard [--seed N] [--alpha A]` —— 统计保证的守卫；无分布保形覆盖率（≥ 1−α）以及 Accept/Abstain/Reject 判定（用于 CCOS 响应守卫）。
- `scirust attest [--seed N]` —— 可验证推理的哈希链证明日志（Freivalds，#80）；验证链条，拒绝伪造的推理，并展示防篡改性（CCOS 事件日志桥接）。
- `scirust quantum [--seed N] [--qubits Q] [--chi C]` —— MPS / Tensor-Train 量子电路模拟器（自研截断 SVD，零 FFI）；展示一个精确的 GHZ 态，并报告键维以及 MPS 内存占用相对稠密 2ⁿ 态矢量的对比。
