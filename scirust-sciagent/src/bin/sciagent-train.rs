...
use scirust_sciagent::train::checkpoint::{latest_checkpoint, load_checkpoint, save_checkpoint, CheckpointMeta};
...
fn main() {
    ...
    let dataset = if let Some(dir) = &args.data_dir {
        ...
        eprintln!("Loaded {} tokens from shard dir {:?}", loader.total_tokens(), dir);
        ...
        for _ in 0..trainer_cfg.grad_accum_steps {
            let (inputs, targets) = dataset
                ...
                .unwrap_or_else(|| {
                    ...
                    dataset.next_batch(trainer_cfg.micro_batch_size)
