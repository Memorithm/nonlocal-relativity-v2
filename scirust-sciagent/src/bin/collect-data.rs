...
#[derive(Parser)]
#[command(name = "collect-data", about = "Tokenize Rust code into packed training shards")]
struct Args {
    #[arg(short, long)]
    input: Vec<String>,
...
fn main() {
    ...
    let tok = BpeTokenizer::load_json(&args.tokenizer)
    ...
    let num_shards = (all_tokens.len() + shard_size - 1) / shard_size;
    ...
    for shard_idx in 0..num_shards {
        ...
        eprintln!("Shard {:04}: {} tokens -> {:?}", shard_idx, shard_data.len(), shard_path);
    }
