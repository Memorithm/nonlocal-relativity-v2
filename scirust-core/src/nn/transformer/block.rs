...
impl TransformerBlock {
    ...
    pub fn state_dict(&self) -> HashMap<String, Tensor> {
        ...
        for (k, v) in self.ln1.state_dict() { map.insert(k, v); }
        for (k, v) in self.mha.state_dict()  { map.insert(k, v); }
        for (k, v) in self.ln2.state_dict()  { map.insert(k, v); }
    ...
    pub fn load_state_dict(&mut self, sd: &HashMap<String, Tensor>) -> std::result::Result<(), String> {
        ...
        let p = &self.name;
        let ffn1_w = sd.get(&format!("{p}.ffn1.weight"))
            .ok_or_else(|| format!("missing key: {p}.ffn1.weight"))?;
        let ffn1_b = sd.get(&format!("{p}.ffn1.bias"))
            .ok_or_else(|| format!("missing key: {p}.ffn1.bias"))?;
        let ffn2_w = sd.get(&format!("{p}.ffn2.weight"))
            .ok_or_else(|| format!("missing key: {p}.ffn2.weight"))?;
        let ffn2_b = sd.get(&format!("{p}.ffn2.bias"))
            .ok_or_else(|| format!("missing key: {p}.ffn2.bias"))?;

        self.ffn1.weight = ffn1_w.clone(); self.ffn1.bias = ffn1_b.clone();
        self.ffn2.weight = ffn2_w.clone(); self.ffn2.bias = ffn2_b.clone();
        Ok(())
    }
}
