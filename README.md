...
- **Industrial & automotive monitoring** (`scirust-signal`, `scirust-opcua`, `scirust-mqtt`, `scirust-pdm`, `scirust-mlops`, `scirust-func-safety`, `scirust-integration`) — signal processing (FFT, windows, bearing BPFO/BPFI/BSF diagnostics, order analysis, **radar** — pulse-compression (LFM chirp / Barker codes, matched filtering with exact energy/peak-to-sidelobe oracles), the **ambiguity function** (joint delay-Doppler response with the LFM range-Doppler coupling ridge), **Doppler processing** (range-Doppler map — stationary vs. moving targets separated into zero- vs. non-zero-Doppler bins), **MTI clutter cancellers** (binomial pulse cancellers with an exact DC null), **CFAR detection** (cell-averaging & ordered-statistic, the latter robust to interfering targets that mask CA-CFAR; false-alarm rate validated statistically — plus a **2-D CFAR** over the range-Doppler map with **connected-component clustering** of detections into amplitude-weighted target centroids, then **target tracking** — α–β constant-velocity track filters and a nearest-neighbour multi-target tracker with track birth/death, plus a full constant-velocity **Kalman filter** (adaptive gain, live covariance, innovation likelihood) and an **IMM** (Interacting Multiple Model) estimator that blends a quiet and an agile model by Markov mode probabilities to follow manoeuvring targets with less lag than any fixed filter, a **planar coordinated-turn IMM** — a general n-state linear Kalman filter (Cholesky measurement update) blending constant-velocity and constant-turn-rate models to track turning targets in the (x, y) plane, following the arc instead of overshooting it — an **extended Kalman filter** that tracks a Cartesian state directly from raw **polar range/bearing** radar measurements (nonlinear observation linearised by its Jacobian, azimuth innovation wrapped across the ±π boundary), a **multi-target tracker** of per-target EKFs associated by a **statistical NIS (normalised-innovation-squared) validation gate** — a χ²-thresholded Mahalanobis distance that adapts to each track's own uncertainty, with track birth/death — and a **probabilistic data association filter (PDAF)** that tracks through clutter by soft-combining every gated measurement weighted by its association probability (with an innovation-spread covariance term) instead of a brittle hard nearest-neighbour pick — closing the OpenRadar detection→track pipeline), **micro-Doppler analysis** (a Hann-windowed spectrogram of the slow-time return plus ridge / bulk-Doppler / bandwidth / cadence descriptors — the time-frequency micro-motion signature behind non-cooperative target recognition, recovering a rotating scatterer's body Doppler and rotation cadence from a synthetic oracle), **detection statistics** (Swerling I probability of detection `P_fa^{1/(1+SNR)}` and Albersheim's equation for the steady-target case — both `P_d`↔SNR directions — quantifying the fluctuation loss that complements the CFAR threshold), the **radar range equation** (a monostatic link budget — `1/R⁴` received power, `k_B·T·B·F` noise, delivered SNR versus RCS/range, and the maximum detection range that closes with the Swerling required SNR — the radar analog of the EO/IR range budget), **clutter amplitude statistics** (Rayleigh, Weibull and log-normal distributions — the spiky-clutter models CFAR thresholds are designed against, with a self-contained error function; Weibull with shape 2 recovers Rayleigh), and **PRF ambiguities & blind speeds** (unambiguous range `c/(2·PRF)` and velocity `λ·PRF/4`, MTI blind speeds `n·λ·PRF/2`, and range/velocity folding — with the invariant range–velocity ambiguity product `cλ/8`, the pulse-Doppler dilemma), and **stepped-frequency synthetic wideband ranging** (fine range resolution `c/(2·N·Δf)` synthesised by an inverse DFT of a burst of frequency-stepped narrowband pulses — wideband resolution without wideband hardware, as in ISAR/GPR), and **space-time adaptive processing (STAP)** for airborne radar — a joint angle-Doppler adaptive filter `w = R⁻¹s/(sᴴR⁻¹s)` that nulls the ground-clutter ridge `f_d = β·f_s` (clutter collapses onto a 1-D ridge in the 2-D angle-Doppler plane, so a slow mover buried in clutter Doppler but off the ridge is separable), delivering `SINR = P·sᴴR⁻¹s` — deeply notched on the ridge, near the full `NM` coherent gain off it — the flagship technique for detecting slow movers no angle-only or Doppler-only filter can pull out of clutter, and **synthetic-aperture radar (SAR) azimuth compression** — the imaging mode, focusing a point target's quadratic slow-time phase history (an azimuth chirp `exp(−j·2π·(x−x₀)²/(λR))`) by matched filtering to synthesise a `λR/D` aperture, giving the celebrated range-independent `D/2` cross-range resolution (finer for a *smaller* antenna), with the closed-form aperture / Doppler-bandwidth `2v/D` / chirp-rate `2v²/(λR)` relations and point-target focusing and two-target resolution demonstrated against oracles, **array processing** (ULA steering vectors, delay-and-sum beamforming, DOA estimation, high-resolution **MVDR/Capon** — resolving two sources closer than the array beamwidth where the conventional beamformer merges them, via a from-scratch complex Hermitian inverse — **MUSIC** subspace direction finding, built on a from-scratch complex-Hermitian eigensolver (cyclic Jacobi), gridless **ESPRIT** — reading the angles straight off the eigenvalues of the subarray-subspace rotation, resolving off-grid sources without a spectral scan, via a from-scratch complex eigensolver (Hessenberg reduction + shifted QR) — and **amplitude-comparison monopulse** — single-dwell sum/difference angle estimation with the exact `tanh(θ·θ_s/σ²)` discriminator, giving off-boresight angle accuracy far finer than the beamwidth — and **phase-comparison (interferometric) angle estimation** — off-boresight angle read from the phase difference `Δφ = 2π·d·sin θ/λ` across a two-element baseline (`θ = arcsin(Δφ·λ/(2π·d))`), with the resolution vs. unambiguous-field-of-view `arcsin(λ/2d)` trade-off that governs baseline choice, and the phase wrap/alias behaviour a wide baseline induces) and **FMCW / mmWave processing** (beat-frequency ranging, range resolution, and the range-Doppler cube built from raw beat chirps by two FFTs — fast-time range and slow-time Doppler — the automotive/OpenRadar model), and an extensible **denoising & noise-detection** toolkit — linear/rank/transform/variational/adaptive families behind a `Denoiser` trait (Savitzky-Golay, Hampel, Wiener, Haar/Daubechies-4/6/8 wavelet shrinkage with universal or per-level SURE thresholds, spectral subtraction, iterative and **exact** (Condat, KKT-certified) total variation, Kalman RTS smoothers — local-level auto-tuned by innovation whiteness, and local linear-trend — plus LMS/RLS adaptive line enhancers), a noise characterizer that classifies Gaussian/impulsive/periodic/colored/baseline disturbances, and a detect-then-denoise auto pipeline with a residual whiteness self-check that flags when information leaks into the "noise"), OPC-UA PLC connectivity with 8 simulated sensors, MQTT SparkPlug B publishing, predictive maintenance (Health Index, RUL, CUSUM, 4 fault detectors), industrial MLOps (drift, shadow deploy, signed OTA), ISO 26262 functional safety (ASIL A-D, fault injection, degraded mode, hash-chained audit log), integration kit (unified Backend/Pipeline/Config/templates), and a dedicated `scirust-industrial` CLI (discover, test, gen-config, scaffold, run, doctor — plus the vertical demos below).
...
- **Pattern detection** (`scirust-vision`, `scirust-audio`, `scirust-graph`, `scirust-sequential`, `scirust-multivariate`, `scirust-unsupervised`, `scirust-seasonal`, `scirust-nlp-advanced`) — computer vision (CNN, HOG, LBP, Canny, Otsu, NMS, plus **optronics / precision-optics imaging** — Gaussian **and diffraction-limited Airy PSFs** (with the Rayleigh resolution limit `1.22·λ/D`), MTF with the MTF50 resolution metric, **Richardson–Lucy (spatial) and Wiener (frequency-domain) deconvolution** for EO/IR image restoration, ray optics: ABCD ray-transfer matrices with Gaussian-beam propagation via the complex q-parameter for optical-train design, **image-domain small-target CFAR detection** — the EO/IR analogue of the radar CFAR, pulling small hot targets out of a spatially varying thermal background (local guard/training-ring statistics, intensity-weighted sub-pixel centroids) to feed the tracking chain from an IR focal plane, and **thermal-infrared radiometry & sensor sensitivity** — Planck's law, Stefan–Boltzmann `σT⁴` and Wien's peak-wavelength law, band-integrated thermal contrast, the **NETD** (noise-equivalent temperature difference) and **MRTD** (minimum resolvable temperature difference, the NETD-over-MTF trade-off) metrics that spec a thermal imager, and **Beer–Lambert atmospheric transmission & the EO/IR range budget** — path transmittance `e^{−αR}`, extinction from meteorological visibility (Koschmieder), the contrast-transmission law, and the target ΔT required to detect through an attenuating path at range), audio (MFCC, chroma, pitch YIN, onset detection), graph patterns (subgraph isomorphism, motif discovery, community detection, betweenness), sequential (HMM, CRF, DTW, KMP/Boyer-Moore), multivariate (PCA, ICA, K-Means++, MDS, CCA), unsupervised (autoencoder, isolation forest, DBSCAN, LOF, GMM), seasonal (STL, ACF/PACF, Mann-Kendall, CUSUM), NLP (NER, LDA, TextRank, MinHash, NaiveBayes).
...
- **Relativity research layers** (`scirust-relativity`, `scirust-fractional`, `scirust-nonlocal-relativity`) — deterministic fixed-background metrics, Christoffel symbols, and test-particle worldlines, plus an explicitly experimental coordinate-memory extension where the state equation remains ordinary and a Caputo L1 velocity-history force appears on the right-hand side. The nonlocal layer keeps complete deterministic history as the default oracle, exposes explicit memory/history/transport/stepper components, includes an opt-in bounded short-memory approximation, preserves semi-implicit Euler as the default reference path, adds a Heun PECE predictor-corrector option, and reports self-convergence and Schwarzschild exterior diagnostics — plus a **discrete parallel-transported memory** (`DiscreteConnectionTransport`, a Heun predictor-corrector approximation of `dV^mu/dlambda = -Gamma^mu_(alpha beta) u^alpha V^beta` along the accepted worldline, `O(D³·N²)`), an explicit **affine-vs-proper-time parameterization mode** with typed rejection of non-timelike or drifting states (no auto-renormalization), and a **Cartesian-vs-cylindrical Minkowski chart comparison** showing transported memory cuts cross-chart disagreement by orders of magnitude versus raw coordinate memory and that the gap shrinks under refinement, an opt-in **curvature-modulated memory** research hook (`SchwarzschildKretschmannModulator`, an explicitly phenomenological `q = 1 + beta·L⁴·K` reweighting by the Schwarzschild Kretschmann scalar `K = 48M²/r⁶`, bypassed bit-for-bit at `beta = 0`), an **exact flat-spacetime transport oracle** (`exact_cylindrical_minkowski_transport`, a closed-form, non-discretized reference exploiting the path-independence of transport under zero curvature, against which `DiscreteConnectionTransport` is shown to converge at second order — not a general bitensor propagator, and not applicable to curved backgrounds), a **non-uniform-grid Caputo operator** (`scirust_fractional::caputo_l1_nonuniform`, additive alongside the existing uniform-grid scheme) powering a **proper-time-based memory diagnostic** that evaluates Caputo memory against a trajectory's own non-uniform proper-time axis rather than its affine parameter, an **exact curved-background transport oracle** (`exact_schwarzschild_circular_orbit_transport`, a closed-form matrix-exponential solution exploiting the *constancy* of the transport generator along a circular equatorial geodesic orbit — a different mechanism than flat-spacetime path-independence, exact only for that one orbit family, not a general bitensor propagator), a second fixed background and modulator (`scirust_relativity::ReissnerNordstrom`, exact analytic Christoffel symbols reducing to Schwarzschild's at zero charge, plus `ReissnerNordstromFieldModulator`, weighted by the electromagnetic field invariant `F_(mu nu)F^(mu nu) = 2Q²/r⁴` rather than a curvature invariant), an **adaptive-step integrator** (`simulate_nonlocal_worldline_adaptive`, a standard embedded Heun-Euler pair choosing its own non-uniform affine-parameter step and evaluating memory via the non-uniform Caputo operator directly, reaching comparable accuracy to an 800-step fixed run in as few as 23 accepted steps in the shipped example — now composable with transport and modulation via `simulate_nonlocal_worldline_adaptive_with_policy`, reusing the same segment-transport mechanism the fixed-step path uses, verified bit-identical to the pre-composition implementation), and a **rotating background** (`scirust_relativity::Kerr`, standard Boyer-Lindquist coordinates, the only background in this crate whose connection is a finite-difference rather than exact analytic result — a disclosed tradeoff given the far greater complexity of Kerr's Christoffel symbols — validated at zero spin against Schwarzschild and via the correct Lense-Thirring frame-dragging sign, with frame dragging emerging end-to-end from the ordinary geodesic equation on a stationary-observer worldline in the shipped example), and a **second adaptive-step integrator** (`simulate_nonlocal_worldline_adaptive_with_stepper_policy`, the first adaptive path to genuinely reuse this crate's own `MemoryLaw` and `WorldlineStepper` traits rather than a bespoke parallel mechanism — two non-uniform-aware Caputo memory laws plus the existing semi-implicit Euler stepper, error-controlled by classical step-doubling since Euler has no natural embedded higher-order partner; the Heun-PECE stepper is deliberately excluded, with the exact reason disclosed in `adaptive_stepper.rs`'s module documentation — its provisional-point parameter formula assumes uniform step spacing). It does not modify Einstein field equations, stress-energy, or matter-generated curvature, and it is not empirical validation of new physics.
