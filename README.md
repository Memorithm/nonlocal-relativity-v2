...
- **Multi-domain simulation environments** (`scirust-sim`) — the unified "here is a system, step it through time, let an agent interact with it" layer the platform previously lacked: a deterministic time-stepping engine (a `System` trait `y' = f(t, y)` + fixed-step RK4, and a `SecondOrderSystem` trait + **symplectic** semi-implicit Euler — the tests show the two-body orbit staying closed at a step size where explicit Euler visibly spirals out), a gym-style **`Environment` trait** (`reset` / `step(action) → observation, reward, done`, mirroring `scirust-learning::rl::Env`) with the classic **cart-pole** (reference physics constants, seeded bit-replayable episodes) and a deterministic **grid world**, a seeded **SplitMix64** generator validated against the published reference vectors, and oracle-tested ready-made models in eight domains: **mechanics** (spring–mass–damper vs the underdamped closed form, full nonlinear pendulum with energy conservation at large amplitude, projectile with linear drag vs its exact solution), **orbital** (two-body Kepler: energy and angular momentum conserved to 1e-9, circular orbit closing after exactly one Kepler period), **epidemiology** (SIR/SEIR: population conserved to round-off, epidemic threshold at R₀ = 1, the exact transcendental final-size relation), **ecology** (Lotka–Volterra first integral conserved, logistic closed form), **chemistry** (consecutive reactions vs the Bateman solution, reversible reaction relaxing to K = k_f/k_r), **thermal** (Newton cooling, 1-D heat rod validated on the *discrete* eigenmode decay rate and the maximum principle), **electrical** (RC charging, series RLC vs the damped-oscillator closed form + passivity), and **stochastic/queueing** (GBM and Ornstein–Uhlenbeck sampled by their *exact* transition laws, an M/M/1 discrete-event queue matching L = ρ/(1−ρ), W = 1/(μ−λ) and Little's law). Pure Rust, zero dependencies, `#![forbid(unsafe_code)]`, `#![deny(missing_docs)]`, explicit seeds everywhere (bit-reproducible runs), Miri-gated in CI — 66 tests + doctest.
