...
impl CubicSpline {
    ...
    ///
    /// `dy0` is the required first derivative `s'(x_0)` and `dyn` the required
    /// `s'(x_{n-1})`. Requires at least two nodes with strictly increasing,
    /// finite `xs` and finite `ys` of matching length; `dy0` and `dyn` must be
    ...
    pub fn clamped(
        ...
        if !dyn.is_finite() {
            return Err(InterpError::NonFinite { index: xs.len() - 1 });
        }
        let m = solve_moments(xs, ys, BoundaryCondition::Clamped { dy0, dyn });
        Ok(Self {
            xs: xs.to_vec(),
            ys: ys.to_vec(),
            m,
...
fn solve_moments(xs: &[f64], ys: &[f64], bc: BoundaryCondition) -> Vec<f64> {
    ...
    match bc {
        ...
        BoundaryCondition::Clamped { dy0, dyn } => {
            b[0] = 2.0 * h[0];
            c[0] = h[0];
            d[0] = 6.0 * (slope[0] - dy0);
            a[n - 1] = h[n - 2];
            b[n - 1] = 2.0 * h[n - 2];
            d[n - 1] = 6.0 * (dyn - slope[n - 2]);
        }
    }
...
fn solve_moments_two(xs: &[f64], ys: &[f64], bc: BoundaryCondition) -> Vec<f64> {
    match bc {
        ...
        BoundaryCondition::Clamped { dy0, dyn } => {
            ...
            //    h*M0 + 2h*M1 = 6(dyn - slope)
            let r0 = 6.0 * (slope - dy0);
            let r1 = 6.0 * (dyn - slope);
