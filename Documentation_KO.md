...
- `scirust kvcache [--seed N] [--budget B]` — 탄력적 압축 KV 캐시(INT4 기반+잔차 타일, 그룹별 스케일); 압축률과 전체 정밀도 대비 어텐션의 코사인 충실도를 보고하며, `--budget`은 경계가 있는 소프트 페이징을 표시(SLHAv2/CCOS 시너지).
- `scirust guard [--seed N] [--alpha A]` — 통계적으로 보장되는 가드; 분포에 구애받지 않는 컨포멀 커버리지(≥ 1−α)와 Accept/Abstain/Reject 판정을 제공(CCOS 응답 가드용).
- `scirust attest [--seed N]` — 검증 가능한 추론의 해시 체인 어테스테이션 로그(Freivalds, #80); 체인을 검증하고, 위조된 추론을 거부하며, 변조 증거를 표시(CCOS 이벤트 로그 브리지).
- `scirust quantum [--seed N] [--qubits Q] [--chi C]` — MPS / Tensor-Train 양자 회로 시뮬레이터(자체 구현 절단 SVD, FFI 없음); 정확한 GHZ 상태를 표시하고, 본드 차원과 밀집 2ⁿ 상태 벡터 대비 MPS 메모리 사용량을 보고.
